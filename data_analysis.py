...
def load_data(filepath: str = os.path.join(DATA_DIR, LATEST_FILE)) -> pd.DataFrame:
    ...
    for col in ["current_price", "price_change_24h", "market_cap", "total_volume"]:
...
def compute_summary(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    summary = {
        ...
        "average_change_24h": round(df["price_change_24h"].mean(), 2),
...
def top_movers(df: pd.DataFrame, n: int = 3) -> Dict[str, pd.DataFrame]:
    ...
    top_gainers = df.nlargest(n, "price_change_24h")[["name", "symbol", "price_change_24h"]]
    top_losers = df.nsmallest(n, "price_change_24h")[["name", "symbol", "price_change_24h"]]
...
def correlation_matrix(df: pd.DataFrame) -> pd.DataFrame:
    ...
    corr = df[["current_price", "price_change_24h", "market_cap", "total_volume"]].corr()
